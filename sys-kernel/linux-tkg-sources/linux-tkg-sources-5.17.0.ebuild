...
SRC_URI="${GENPATCHES_URI} ${KERNEL_URI} ${ARCH_URI}
		https://github.rc1844.workers.dev/graysky2/kernel_compiler_patch/raw/master/more-uarches-for-kernel-5.17%2B.patch -> more-uarches-for-kernel-${SHPV}%2B-${PV}.patch
		https://github.rc1844.workers.dev/zhmars/cjktty-patches/blob/master/cjktty-add-cjk32x32-font-data.patch?raw=true -> cjktty-add-cjk32x32-font-data.patch
		https://github.rc1844.workers.dev/zhmars/cjktty-patches/blob/master/v5.x/cjktty-5.17.patch?raw=true -> cjktty-${SHPV}.patch
...
src_prepare() {
	...
	if use cjktty; then
		eapply "${DISTDIR}/cjktty-${SHPV}.patch"
		eapply "${DISTDIR}/cjktty-add-cjk32x32-font-data.patch"
	fi
}
